...
export default function HostShellLayout({ children }: { children: React.ReactNode }) {
  return (
    <div className="min-h-screen">
      <header className="border-b border-charcoal/10 bg-white">
        <div className="mx-auto flex max-w-3xl items-center justify-between gap-4 px-6 py-4">
          <Link href="/host" className="flex items-center gap-2.5 no-underline">
            <span className="flex h-8 w-8 items-center justify-center rounded-[8px] bg-sage font-serif text-[13px] font-bold text-honey">
              HR
            </span>
            <span className="font-serif text-base font-bold text-sage">Host</span>
          </Link>
